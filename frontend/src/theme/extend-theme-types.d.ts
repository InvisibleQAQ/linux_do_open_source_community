// `@mui/x-date-pickers` is a TYPE-ONLY devDependency here.
// Minimal's `core/components/text-field.tsx` styles the picker input slots
// (MuiPickersInputBase / MuiPickersOutlinedInput / MuiPickersFilledInput),
// so those theme keys must exist at typecheck time. No runtime import exists,
// so the package is fully erased from the bundle.
